...
export function Vacinas(props:VacinaProps) {
    ...
    const [aplicacoesEvents, setAplicacoesEvents] = useState<EventInput[]>([]);
    ...
    const getCartaoVacinaByPet = async (petID:number) => {
        ...
        if(responseCartao.status>=200 && responseCartao.status<=300) {
            ...
            aplicacoesEvents.splice(0,aplicacoesEvents.length);
            for(let i=0; i<aplicacoesTemp.length ;i++) {
                aplicacoesEvents.push({
                    id: aplicacoesTemp[i].aplicacaoVacinaID!.toString(),
                    title: aplicacoesTemp[i].vacina!.nome,
                    start: aplicacoesTemp[i].dtAplicada!=null? moment(aplicacoesTemp[i].dtAplicada).format('yyyy-MM-DD') : moment(aplicacoesTemp[i].dtProgramada).format('yyyy-MM-DD'),
                })
            }
            setAplicacoesEvents(aplicacoesEvents);
        }
    }    
